...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      stock: product.stock,
      image:
        product.image && product.imageType
...
export async function PUT(
  ...
  try {
    ...
    const stock = formData.get("stock") as string       // ✅ keep as string (it's an enum)
    ...
    // ✅ FIX: validate as StockStatus enum, never parseInt
    if (stock) {
      if (!Object.values(StockStatus).includes(stock as StockStatus)) {
        return NextResponse.json(
          { error: `Valeur de stock invalide. Valeurs acceptées: ${Object.values(StockStatus).join(", ")}` },
          { status: 400 }
        )
      }
      updateData.stock = stock as StockStatus
    }
